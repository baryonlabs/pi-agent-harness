...
각 테스트 프롬프트에 대해 pi의 번들된 `subagent` 도구를 **parallel 모드**로 사용하여 두 위임을 **동시에** 실행한다:

**With-skill 위임:**
```
프롬프트: "{테스트 프롬프트}"
스킬 로드: task에 /skill:{스킬 이름} 로드 (또는 에이전트 본문에 스킬 지시 포함)
...
두 위임을 `subagent` 도구의 parallel 모드로 한 번에 실행하면, 동일한 조건에서 스킬의 유무만 다르게 둘 수 있어 비교의 공정성이 확보된다.
...
pi `subagent` 도구는 위임 완료 시 사용량 통계(turns, ↑input ↓output, cache, $cost, ctx)를 표시한다. 이 표시에서 `total_tokens`와 `duration_ms`에 해당하는 값을 **즉시** 저장한다. 이 데이터는 완료 표시 시점에만 접근 가능하고 이후 복구할 수 없다.
...
테스트/평가 과정에서 전문 역할의 에이전트를 활용하면 품질이 향상된다. 각 역할은 `.pi/agents/`에 별도 에이전트로 정의하거나, `subagent` 도구로 해당 역할 지시를 담아 호출한다.
...
2. **무게를 벌지 않는 것은 제거하라** — 트랜스크립트를 읽고, 스킬이 모델에게 비생산적인 작업을 시키고 있다면 해당 부분을 삭제한다.
...
> 이 과정은 `pi -p`(print 모드)를 사용하는 자동화 스크립트로 수행한다. 토큰 비용이 높으므로 스킬이 충분히 안정화된 후 최종 단계에서 실행한다.
