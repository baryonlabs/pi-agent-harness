...
pi는 [Agent Skills 표준](https://agentskills.io/specification)을 따른다. SKILL.md 형식은 Claude Code와 거의 동일하며, 스킬은 `.pi/skills/`(프로젝트) 또는 `~/.pi/agent/skills/`(글로벌)에 위치한다.
...
Description은 스킬의 주된 트리거 메커니즘이다. pi는 시스템 프롬프트에 주입된 스킬 목록(name + description)을 보고 스킬 사용 여부를 결정한다. 강제 로딩이 필요하면 사용자가 `/skill:name` 커맨드로 직접 호출할 수도 있다.

frontmatter 검증 규칙: `name`은 최대 64자(소문자/숫자/하이픈만), `description`은 최대 1024자.

### 트리거 메커니즘 이해

모델은 자신의 기본 도구로 쉽게 처리할 수 있는 단순 작업에는 스킬을 호출하지 않는 경향이 있다. "이 PDF 읽어줘" 같은 단순 요청은 description이 완벽해도 트리거되지 않을 수 있다. 복잡하고 다단계이며 전문적인 작업일수록 스킬 트리거 확률이 높다. 확실히 특정 스킬을 쓰게 하려면 사용자가 `/skill:name`으로 명시 호출한다.

### 작성 원칙

1. **스킬이 하는 일** + **구체적 트리거 상황**을 모두 기술
2. 유사하지만 트리거하면 안 되는 경우를 구분하는 경계 조건 명시
3. 약간 "pushy"하게 — 모델이 트리거를 보수적으로 판단하는 경향을 보상
...
- "모델이 이미 알고 있는 내용인가?" → 삭제
- "이 설명이 없으면 모델이 실수하는가?" → 유지
...
서브에이전트 실행에 pi의 subagent 도구를 사용한 경우, 도구가 완료 시 표시하는 사용량 통계(turns/tokens/cost)에서 토큰 수와 소요 시간을 캡처해 즉시 저장한다. 이 데이터는 완료 시점에만 접근 가능하고 이후 복구가 어렵다.
...
신규 스킬 생성 전, 기존 스킬과의 중복을 확인한다. 하네스를 반복 구축하다 보면 기능이 겹치는 스킬이 다른 이름으로 누적되기 쉽다. `.pi/skills/`(및 글로벌 `~/.pi/agent/skills/`)를 먼저 살펴본다.
